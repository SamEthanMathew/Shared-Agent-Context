...
# Provider marks, inlined rather than linked. A strict CSP (default-src 'self')
# blocks every external image, so a hosted logo URL would render as a broken icon
# in production while looking correct locally. These are each vendor's own mark,
# which their branding terms require for a "continue with" button.
_MARK_GOOGLE = (
    '<svg viewBox="0 0 48 48" aria-hidden="true" focusable="false">'
    '<path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38'
    ' 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24'
    ' 9.5z"/>'
    ...
    '<path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.28-3.14.76-'
    '4.59l-7.97-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/>'
    '<path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45'
    '-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48'
    ' 24 48z"/></svg>'
)
_MARK_GITHUB = (
    '<svg viewBox="0 0 16 16" fill="currentColor" aria-hidden="true" '
    'focusable="false"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4'
    '.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48'
    ...
    '-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55'
    '.38A8.01 8.01 0 0016 8c0-4.42-3.58-8-8-8z"/></svg>'
)
_MARKS = {"google": _MARK_GOOGLE, "github": _MARK_GITHUB}


def _sso_buttons(carry: str) -> str:
    """Provider buttons, or nothing at all when none are configured.

    The anchor *is* the button rather than wrapping one: a <button> inside an <a>
    is invalid HTML, and browsers disagree about which one takes the click, so
    keyboard activation lands somewhere different from a mouse press.
    """
    ...
    if not providers:
        return ""
    links = "".join(
        f'<a class="sso-btn" href="/auth/sso/{p.name}/start{carry}">'
        f'{_MARKS.get(p.name, "")}'
        f"<span>Continue with {html.escape(p.label)}</span></a>"
        for p in providers
    )
    return f'<div class="sso">{links}</div><p class="muted centered">or</p>'
